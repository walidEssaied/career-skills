...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 sm:px-0 mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">User Courses</h1>
            <p class="mt-1 text-sm text-gray-600">Courses for {{ $user->name }}</p>
        </div>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                @if($course->pivot->status === 'not_started') bg-gray-100 text-gray-800
                                @elseif($course->pivot->status === 'in_progress') bg-yellow-100 text-yellow-800
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="w-full bg-gray-200 rounded-full h-2.5">
                                <div class="bg-indigo-600 h-2.5 rounded-full" style="width: {{ $course->pivot->progress }}%"></div>
                            </div>
                            <span class="text-sm text-gray-500">{{ $course->pivot->progress }}%</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $course->pivot->completion_date ? $course->pivot->completion_date->format('M d, Y') : '-' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <button onclick="document.getElementById('editCourseModal{{ $course->id }}').classList.remove('hidden')"
                                class="text-indigo-600 hover:text-indigo-900 mr-3">Update Progress</button>
                            <form action="{{ route('admin.users.courses.destroy', [$user, $course]) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                <div>
                    <div class="mt-3 text-center sm:mt-0 sm:text-left">
                        ...
                        <div class="mt-4">
                            ...
                            <input type="number" name="progress" id="progress" min="0" max="100" value="0" required
